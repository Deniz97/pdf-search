"""Cache layer for pdf2image, OCR, and PP-Structure results, keyed by document DB id."""
...
def _ppstructure_path(doc_id: str) -> Path:
    return _cache_root(doc_id) / "ppstructure.json"


def has_cached_ppstructure(doc_id: str) -> bool:
    """Check if PP-Structure blocks are cached for this document."""
    return _ppstructure_path(doc_id).exists()


def _to_json_safe(obj):
    """Convert numpy arrays etc. to JSON-serializable form."""
...
def save_ppstructure(doc_id: str, pages_blocks: list[tuple[int, list[dict]]]) -> None:
    """Save PP-Structure results. Format: [[page_num, blocks], ...]."""
    _cache_root(doc_id).mkdir(parents=True, exist_ok=True)
    data = [
        [page_num, [_to_json_safe(b) for b in blocks]]
        for page_num, blocks in pages_blocks
    ]
    _ppstructure_path(doc_id).write_text(
        json.dumps(data, ensure_ascii=False, indent=None),
        encoding="utf-8",
    )


def load_ppstructure(doc_id: str) -> list[tuple[int, list[dict]]]:
    """Load PP-Structure results."""
    data = json.loads(_ppstructure_path(doc_id).read_text(encoding="utf-8"))
    return [(int(p[0]), p[1]) for p in data]
